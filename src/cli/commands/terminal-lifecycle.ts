...
async function worktree(process: ProcessAdapter, value: string | undefined): Promise<Result<string>> { const path = resolve(value ?? process.cwd()); const result = await process.run("git", ["-C", path, "rev-parse", "--show-toplevel"]); return result.kind === "ok" ? ok(result.value.stdout.trim()) : failed(`worktree path is not a Git directory: ${value ?? process.cwd()}`); }
...
export async function executeTerminalLifecycle(args: readonly string[], env: Environment, process: ProcessAdapter): Promise<Result<string>> {
  const operation = args[0]; const rest = args.slice(1); let json = false; let selector: string | undefined; let command: string | undefined; let title: string | null = null; let worktreePath: string | undefined;
  for (let index = 0; index < rest.length; index += 1) { const arg = rest[index]; if (arg === "--json") json = true; else if (["--command", "--title", "--worktree", "--wait-port", "--timeout"].includes(arg)) { const value = rest[++index]; if (value === undefined) return failed(`${arg} requires a value`, 2); if (arg === "--command") command = value; else if (arg === "--title") title = value; else if (arg === "--worktree") worktreePath = value; } else if (!arg.startsWith("-") && selector === undefined) selector = arg; else return failed(`unknown terminal ${operation} option: ${arg}`, 2); }
  if (operation === "create") { const path = await worktree(process, worktreePath); if (path.kind !== "ok") return path; const currentHost = host(env); const workspaceId = env.SUPERSET_WORKSPACE_ID ?? null; if (!command) return failed("terminal create requires --command in the TypeScript implementation", 2); const call = hostCommand(currentHost, "create", { workspaceId, command }); if (!call) return failed("cannot create terminal from unknown orchestration host"); const result = await process.run(call.command, call.args); if (result.kind !== "ok") return result; const value = parsed(result.value.stdout) ?? {}; const id = hostId(value); if (!id) return failed(`${currentHost} terminal create returned no terminal identity`); const record: TerminalLifecycleRecord = { terminalId: id, host: currentHost, workspaceId, worktree: path.value, title, command, createdAt: new Date().toISOString(), pid: jsonNumber(value, ["pid", "processId", "terminal.pid", "result.pid"]), rootPid: jsonNumber(value, ["rootPid", "processRootPid", "terminal.rootPid", "result.rootPid"]), port: jsonNumber(value, ["port", "terminal.port", "result.port"]) }; await save(env, record); return ok(output({ host: currentHost, worktree: path.value, title, terminalId: id, pid: record.pid, rootPid: record.rootPid, port: record.port }, json)); }
  const all = await records(env); if (!selector) return failed(`terminal ${operation} requires a selector`, 2); const record = resolveTerminalSelector(all, selector); if (!record) return failed("terminal selector could not be resolved");
  ...
  if (operation === "restart") {
    ...
    if (root === null) return failed(`terminal ${selector} has no recorded process identity; refusing to kill an unowned process`);
