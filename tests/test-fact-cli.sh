...
run_pair() {
  ...
  if shell_out="$(env -i HOME="$work_dir/home" PATH="/usr/bin:/bin" MEGABRAIN_ROOT="$root" MEGABRAIN_FACTS_FILE="$facts" MEGABRAIN_FACT_IMPLEMENTATION=shell "$root/megabrain" "$@" 2>&1)"; then shell_rc=0; else shell_rc=$?; fi
  if binary_out="$(env -i HOME="$work_dir/home" PATH="/usr/bin:/bin" MEGABRAIN_ROOT="$root" MEGABRAIN_FACTS_FILE="$facts" "$root/.build/megabrain" "$@" 2>&1)"; then binary_rc=0; else binary_rc=$?; fi
