...
run_binary() {
  ...
  if MEGABRAIN_STATE_DIR="$state" "$fixture/megabrain" chain list --json >"$output_file" 2>"$error_file"; then
    status=0
  else
    status=$?
  fi
...
MEGABRAIN_STATE_DIR="$state" "$fixture/megabrain" chain list --json >"$bypass_output" 2>"$bypass_error" || fail "deliberate unvalidated opt-out was rejected: $(cat "$bypass_error")"
...
if MEGABRAIN_STATE_DIR="$state" "$routing_fixture/megabrain" chain list --json >"$work/routing.out" 2>&1; then
...
MEGABRAIN_CHAIN_IMPLEMENTATION=shell MEGABRAIN_STATE_DIR="$state" "$fixture/megabrain" chain list --json >"$work/shell.out" 2>"$work/shell.err" || fail "shell chain implementation failed: $(cat "$work/shell.err")"
