...
run_binary() {
  "$root/.build/megabrain" native "$@"
}

printf 'scenario: ensure waits for a usable phone simulator\n'
rm -f "$SIMCTL_BOOT_MARKER" "$SIMCTL_LOG"
output="$(run_binary sim ensure phone --device phone-1 --timeout 1)" || fail "ensure failed: $output"
...
rm -f "$SIMCTL_LOG"
phone_output="$(run_binary app reload phone --route deep/link --bundle-id com.example.phone --url-template 'exp://127.0.0.1:{metro_port}/--/{route}' --device iPhone --metro-port 8082 --timeout 1)" || fail "phone reload failed: $phone_output"
...
rm -f "$SIMCTL_LOG"
tv_output="$(run_binary app reload tv --route deep/link --bundle-id com.example.tv --url-template 'canto:///{route}' --device tv-1 --metro-port none --timeout 1)" || fail "tv reload failed: $tv_output"
...
output="$(SIMCTL_TERMINATE_STATUS=3 SIMCTL_TERMINATE_ERROR='Process is not running' run_binary app reload phone --route route --bundle-id com.example.phone --url-template 'exp://127.0.0.1:{metro_port}/--/{route}' --device phone-1 --metro-port 8082 --timeout 1)" || fail "reload treated a stopped app as an error: $output"
...
EOF
rm -f "$SIMCTL_LOG"
config_output="$(cd "$work_dir/worktree" && run_binary app reload phone --route configured --timeout 1)" || fail "config reload failed: $config_output"
...
rm -f "$SIMCTL_BOOT_MARKER" "$SIMCTL_LOG"
list_output="$(run_binary sim list phone)" || fail "phone list failed: $list_output"
...
list_json="$(run_binary sim list tv --json)" || fail "tv list failed: $list_json"
...
name_output="$(run_binary app reload phone --route named --bundle-id com.named.phone --url-template 'exp://127.0.0.1:{metro_port}/--/{route}' --device iPhone --metro-port 8082 --timeout 1)" || fail "name selection failed: $name_output"
assert_contains "$(cat "$SIMCTL_LOG")" 'simctl openurl phone-1 exp://127.0.0.1:8082/--/named'
if output="$(run_binary sim ensure phone --device 'Missing iPhone' --timeout 1 2>&1)"; then fail 'missing device name unexpectedly succeeded'; fi
assert_contains "$output" 'no iOS simulator matches name Missing iPhone'
...
if output="$(run_binary sim ensure phone --device iPhone --timeout 1 2>&1)"; then fail 'ambiguous device name unexpectedly succeeded'; fi
...
rm -f "$SIMCTL_BOOT_MARKER" "$SIMCTL_LOG"
configured_output="$(cd "$work_dir/worktree" && run_binary sim ensure phone --timeout 1)" || fail "configured ensure failed: $configured_output"
...
if output="$(run_binary sim ensure phone --timeout 1 2>&1)"; then fail 'zero matching devices unexpectedly succeeded'; fi
assert_contains "$output" 'no iOS simulator matches'
SIMCTL_DEVICES_JSON="$work_dir/multiple.json"
if output="$(run_binary sim ensure phone --timeout 1 2>&1)"; then fail 'multiple devices unexpectedly succeeded'; fi
assert_contains "$output" 'more than one iOS simulator matches'
SIMCTL_DEVICES_JSON="$work_dir/shutdown.json"
SIMCTL_BOOT_STATUS=1
if output="$(run_binary sim ensure phone --device phone-1 --timeout 1 2>&1)"; then fail 'boot failure unexpectedly succeeded'; fi
assert_contains "$output" 'failed to boot simulator phone-1'
SIMCTL_BOOT_STATUS=0
SIMCTL_BOOTED_JSON="$work_dir/shutdown.json"
rm -f "$SIMCTL_BOOT_MARKER"
if output="$(run_binary sim ensure phone --device phone-1 --timeout 1 2>&1)"; then fail 'boot timeout unexpectedly succeeded'; fi
assert_contains "$output" 'timed out waiting for simulator phone-1'
rm -f "$SIMCTL_BOOT_MARKER"
SIMCTL_DEVICES_JSON="$work_dir/shutdown.json"
if output="$(run_binary app reload phone --bundle-id com.example.phone --url-template 'exp://127.0.0.1:{metro_port}/--/{route}' --metro-port 8082 --timeout 1 2>&1)"; then fail 'reload without a booted simulator unexpectedly succeeded'; fi
assert_contains "$output" 'no booted iOS simulator'
SIMCTL_DEVICES_JSON="$work_dir/booted.json"
SIMCTL_METRO_READY=false
if output="$(run_binary app reload phone --bundle-id com.example.phone --url-template 'exp://127.0.0.1:{metro_port}/--/{route}' --device phone-1 --metro-port 8082 --timeout 1 2>&1)"; then fail 'Metro failure unexpectedly succeeded'; fi
